...
Исправлен баг с переносом выделенного текста: при повторном перемещении выделенного диапазона больше не теряется по одной строке выделения. Цельное построчное выделение теперь корректно восстанавливается после перемещения вверх или вниз.
...
Хоткеи `Alt+←/→` выбраны намеренно вместо `Ctrl+Shift+←/→`: `Ctrl+Shift+←/→` конфликтуют со стандартным выделением текста по словам в любом текстовом поле.
...
Плагин видит коллизии хоткеев внутри Obsidian. Глобальные перехваты Windows, macOS или других приложений изнутри Obsidian проверить нельзя.
